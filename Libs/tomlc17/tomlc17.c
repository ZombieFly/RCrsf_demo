  ...
  if (toml_option.check_utf8) {
    ...
    for (int i = 0; i < len;) {
      ...
      if (0xD800 <= ch && ch <= 0xDFFF) {
        ...
        snprintf(result.errmsg, sizeof(result.errmsg),
                 "invalid UTF8 char \\u%04x on line %d", (unsigned int)ch, line);
        goto bail;
      }
      line += (ch == '\n' ? 1 : 0);
      i += n;
...
static int process_numstr(char *buffer, int base, const char **reason) {
  ...
  if (base == 10) {
    ...
    q = buffer;
    q += (*q == '+' || *q == '-') ? 1 : 0;
    if (((const char *)q)[0] == '0' && isdigit((unsigned char)((const char *)q)[1])) {
...
static int scan_literal(scanner_t *sp, token_t *tok) {
  *tok = mktoken(sp, TOK_LIT);
  const char *p = sp->cur;
  while (p < sp->endp && (isalnum((unsigned char)*p) || *p == '_' || *p == '-')) {
    p++;
  }
